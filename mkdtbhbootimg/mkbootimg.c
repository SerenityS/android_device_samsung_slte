...
/* Samsung S LTE EUR revision 09 board's dts:
 * model = "Samsung S LTE EUR revision 09 board(EVT1) based on EXYNOS5430 with Intel modem";
 * model_info-chip = <5430>;
 * model_info-platform = "slte";
 * model_info-subtype = "slte_eur_open";
 * model_info-hw_rev = <9>;
 * model_info-hw_rev_end = <255>;
 * compatible = "samsung,S LTE EUR,r09", "samsung,exynos5430";
 */
...
static void *load_dtbh_block(const char *dtb_path, unsigned pagesize, unsigned *_sz)
{
    ...
    /* The size of the dt header is now known, calculate the entry offsets. */
    offset = hdr_sz;
...
#ifdef DTBTOOL
    if (dt_dir == 0) {
        fprintf(stderr,"error: no dtb directory\n");
        return usage();
    }
#else
    if(kernel_fn == 0) {
